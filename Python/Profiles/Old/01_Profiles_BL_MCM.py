...
dwpoy=243.04*(np.log(relhum/100)+((17.625*temp)/(243.04+temp)))/(17.625-np.log(relhum/100)-((17.625*temp)/(243.04+temp)))
...
wspd=bom[:,7,:].reshape(ni[0],ni[2]) #Knots????????????????????????????????????
...
for j in range(-10,10):
    ...
    min_meanDP_yotc[j]=np.mean(minrh1[0:19,j])
    min_stdDP_yotc[j]=np.std(minrh1[0:19,j])
...
ax0.set_ylabel('Mean minimun relative humidity (%)', size=18)
