...
#*****************************************************************************\
    ...
    Mrelhum_era=np.empty([len(ncount),len(pres_ei)])*np.nan
    RH=np.empty([max(ncount),len(pres_ei)])*np.nan
    ...
    for j in range(-10,10):
        ...
        for i in range(0, len(df)):
            if df['catdist_fron'][i]==j:
                ...
                #RH[k2,:]=np.array(df['mixr'][i])
                k2=k2+1
            Mrelhum_era[k1,:]=np.nanmean(RH, axis=0)
        k1=k1+1
        k2=0
...
#*****************************************************************************\
    ...
    # RHmac=np.empty([max(ncount),max_lev,len(ncount)])*np.nan
    # RHera=np.empty([max(ncount),max_lev,len(ncount)])*np.nan
    # RHD=np.empty([max(ncount),max_lev,len(ncount)])*np.nan
    ...
    for j in range(-10,10):
        for i in range(0, len(df)):
            if df['catdist_fron'][i]==j:
                #RHmac[k1,:,j]=np.array(df['rh MAC'][i])
                #RHera[k1,:,j]=np.array(df['rh ERA'][i])
                #RHD[k1,:,j]=np.array(df['rh MAC'][i])-np.array(df['rh ERA'][i])
                RHmac[k1,:,j]=np.array(df[name_var_era[m]][i])
    ...
    #Combination MAC and ERA to calculate Monte Carlo
...
#*****************************************************************************\
    ...
    n_rd=65 #Number of cases to take
    n_rdyi=70 #Number of cases to take Yi
    n_mean=1000 #Number of means
    RH=RHD #Differences


    RH_rd=np.empty([n_rd,max_lev,len(ncount)])*np.nan
    ...
    RHD_rdmeanera=np.empty([n_mean,max_lev,len(ncount)])*np.nan
    RHD_rdmeanmac=np.empty([n_mean,max_lev,len(ncount)])*np.nan


    RH_rdera_yi=np.empty([n_rdyi,max_lev,len(ncount)])*np.nan
    RH_rdmac_yi=np.empty([n_rdyi,max_lev,len(ncount)])*np.nan
    RHD_rdmeanera_yi=np.empty([n_mean,max_lev,len(ncount)])*np.nan
    RHD_rdmeanmac_yi=np.empty([n_mean,max_lev,len(ncount)])*np.nan


    for i in range(0,n_mean):
        for dis in range(0,len(ncount)):
            for lev in range(0,max_lev):
                    RH_rd[:,lev,dis]=rd.sample(RH[~np.isnan(RH[:,lev,dis]),lev,dis], n_rd)
        ...
        RHD_rdmeanmac[i,:,:]=np.nanmean(RH_rdmac,axis=0) #Random means MAC
        RHD_rdmeanera[i,:,:]=np.nanmean(RH_rdera,axis=0) #Random means ERA
        #Yi's Method
        RHD_rdmeanera_yi[i,:,:]=np.nanmean(RH_rdera_yi,axis=0) #Random means ERA
        RHD_rdmeanmac_yi[i,:,:]=np.nanmean(RH_rdmac_yi,axis=0) #Random means MAC
        #(1000,17,20)

#*****************************************************************************\
    #Rescale
    nlev=17
    height_new=np.arange(0,5501,250) #(23,)
    height_new[0]=100
    h_ei=hght_ei[0:nlev]

    Mrelhum_mac_lev=RHD_rdmeanmac[:,0:nlev,:]
    Mrelhum_era_lev=RHD_rdmeanera[:,0:nlev,:]
    Mrelhum_dif_lev=RHD_rdmean[:,0:nlev,:] #cambio de niveles
    n1,n2,n3=Mrelhum_mac_lev.shape


    Mrelhum_mac_lev_yi=RHD_rdmeanmac_yi[:,0:nlev,:]
    Mrelhum_era_lev_yi=RHD_rdmeanera_yi[:,0:nlev,:]

    #Interpolation
    Mrh_macint=np.empty([n1,len(height_new),n3])*np.nan
    Mrh_eraint=np.empty([n1,len(height_new),n3])*np.nan
    Mrh_difint=np.empty([n1,len(height_new),n3])*np.nan

    Mrh_macint_yi=np.empty([n1,len(height_new),n3])*np.nan
    Mrh_eraint_yi=np.empty([n1,len(height_new),n3])*np.nan
    ...
    for i in range(0,n1):
        for j in range(0,n3):
            y=Mrelhum_mac_lev[i,:,j]
            Mrh_macint[i,:,j]= sp.interpolate.interp1d(x, y,kind='cubic')(new_x)
            del y
            y=Mrelhum_era_lev[i,:,j]
            Mrh_eraint[i,:,j]= sp.interpolate.interp1d(x, y,kind='cubic')(new_x)
            del y
            y=Mrelhum_dif_lev[i,:,j]
            Mrh_difint[i,:,j]= sp.interpolate.interp1d(x, y,kind='cubic')(new_x)
            del y
            #Yi's Method
            y=Mrelhum_mac_lev_yi[i,:,j]
            Mrh_macint_yi[i,:,j]= sp.interpolate.interp1d(x, y,kind='cubic')(new_x)
            del y
            y=Mrelhum_era_lev_yi[i,:,j]
            Mrh_eraint_yi[i,:,j]= sp.interpolate.interp1d(x, y,kind='cubic')(new_x)
...
#*****************************************************************************\
    ...
    RH_meandif=Mrelhum_mac-Mrelhum_era
    RH_meandif=RH_meandif.T
    RH_meandif_lev=RH_meandif[0:nlev,:]

    Mrelhum_mac=Mrelhum_mac[:,0:nlev].T
    Mrelhum_era=Mrelhum_era[:,0:nlev].T

    n1,n2=RH_meandif.shape

    #Interpolation
    RH_meandifint=np.empty([len(height_new),n2])*np.nan
    RH_eraint=np.empty([len(height_new),n2])*np.nan
    RH_macint=np.empty([len(height_new),n2])*np.nan
    ...
    for i in range(0,n2):
        y=RH_meandif_lev[:,i]
        RH_meandifint[:,i]= sp.interpolate.interp1d(x, y,kind='cubic')(new_x)
        del y
        y=Mrelhum_era[:,i]
        RH_eraint[:,i]= sp.interpolate.interp1d(x, y,kind='cubic')(new_x)
        del y
        y=Mrelhum_mac[:,i]
        RH_macint[:,i]= sp.interpolate.interp1d(x, y,kind='cubic')(new_x)
        del y


#*****************************************************************************\
#Rename Interpolation to Originals

    RH_meandif=RH_meandifint #General Diff
    RHD_rdmeanmac=Mrh_macint
    RHD_rdmeanera=Mrh_eraint
    RHD_rdmean=Mrh_difint

    nlev,_=RH_meandif.shape

    RHD_rdmeanmac_yi=Mrh_macint_yi
    RHD_rdmeanera_yi=Mrh_eraint_yi

#*****************************************************************************\
# Method 1 (Bootstrapping Yi) (Huang et al., 2014)
#*****************************************************************************\

    tmask=np.empty([nlev,len(ncount)])*np.nan

    for lev in range(0,nlev):
        for dis in range(0,len(ncount)):
            Xo=RH_meandif[lev,dis] #Media del total
            x=RHD_rdmeanmac_yi[:,lev,dis]-RHD_rdmeanera_yi[:,lev,dis]

            x_mean=np.nanmean(RHD_rdmeanmac_yi[:,lev,dis]-RHD_rdmeanera_yi[:,lev,dis])
            x_std=np.nanstd(RHD_rdmeanmac_yi[:,lev,dis]-RHD_rdmeanera_yi[:,lev,dis])
...
#*****************************************************************************\
    tmask2=np.empty([nlev,len(ncount)])*np.nan
...
#*****************************************************************************\
# Method 2  (Bootstrapping t-test)
#*****************************************************************************\
    tmask3=np.empty([nlev,len(ncount)])*np.nan
    from scipy.stats import ttest_ind, ttest_rel

    for lev in range(0,nlev):
        for dis in range(0,len(ncount)):
            a=RHD_rdmeanmac[:,lev,dis]
            b=RHD_rdmeanera[:,lev,dis]
...
#*****************************************************************************\
    ...
    # Xo=RH_meandif[i,j]
    # x=RHD_rdmean[:,i,j]
    # x_mean=np.nanmean(RHD_rdmean[:,i,j])
    # x_std=np.nanstd(RHD_rdmean[:,i,j])
    # CI= sp.stats.norm.interval(0.95,loc=x_mean,scale=x_std)
...
#*****************************************************************************\
    ...
    xtick=[0, height_new[0],height_new[4],height_new[8],height_new[12],height_new[16],height_new[20],height_new[22],6000]
...
#*****************************************************************************\
    ...
    tmask2=tmask2[...,::-1]
    tmask3=tmask3[...,::-1]
    RH_macint=RH_macint[...,::-1]
    RH_eraint=RH_eraint[...,::-1]
#*****************************************************************************\
...
#*****************************************************************************\
    ...
    ax.set_title(name_var_sim[m]+' significant differences (Huang et al., 2015)', size=18)
...
#*****************************************************************************\
# Mask Method 2 (Bootstrapping t-test)
#*****************************************************************************\
...
#*****************************************************************************\
    ...
    RH_ssd=ma.masked_array((RH_macint-RH_eraint),mask=tmask)
    ...
    img = ax.pcolor(RH_ssd,cmap=cmap, vmin=z_mini, vmax=z_maxi)
    div = make_axes_locatable(ax)
    ...
    ax.set_title(name_var_all[m]+' Differences (MAC-ERA-I)', size=18)
    ax.set_xticklabels(np.arange(-15,15,5))
    ax.set_yticklabels(xtick)
    ax.set_ylabel('Altitude (m)', size=18)
    ax.set_xlabel('Position across cold front from cold to warm sector (deg)', size=18)
...
#*****************************************************************************\
    ...
    #z_max=100
    fig, ax = plt.subplots(facecolor='w', figsize=(12,6))
    img = ax.pcolor(RH_macint,cmap='jet', vmin=z_min[m], vmax=z_max[m])
...
#*****************************************************************************\
    ...
    fig, ax = plt.subplots(facecolor='w', figsize=(12,6))
    img = ax.pcolor(RH_eraint,cmap='jet', vmin=z_min[m], vmax=z_max[m])

    div = make_axes_locatable(ax)
    cax = div.append_axes("right", size="6%", pad=0.05)
    cbar = plt.colorbar(img, cax=cax, format=formati[m])
    cbar.ax.set_title('  '+units_var[m], size=12)
    #ax.set_title('Relative Humidity (ERA-I)', size=18)
    ax.set_title(name_var_all[m]+' (ERA-I)', size=18)
    ax.set_xticklabels(np.arange(-15,15,5))
    ax.set_yticklabels(xtick)
    ax.set_ylabel('Altitude (m)', size=18)
    ax.set_xlabel('Position across cold front from cold to warm sector (deg)', size=18)
