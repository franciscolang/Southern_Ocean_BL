...
# temp_ei=temp_ei[:,4:21]
# u_ei=u_ei[:,4:21]
# v_ei=v_ei[:,4:21]
# q_ei=q_ei[:,4:21]
# mixr_ei=mixr_ei[:,4:21]
# rh_ei=rh_ei[:,4:21]
# dthetav_ei=dthetav_ei[:,4:21]
# thetav_ei=thetav_ei[:,4:21]
# theta_ei=theta_ei[:,4:21]
# vshear_ei=vshear_ei[:,4:21]
...
temp_pres=np.empty((len(pres_ei),ni[2]),'float')*np.nan
mixr_pres=np.empty((len(pres_ei),ni[2]),'float')*np.nan
u_pres=np.empty((len(pres_ei),ni[2]),'float')*np.nan
v_pres=np.empty((len(pres_ei),ni[2]),'float')*np.nan
relh_pres=np.empty((len(pres_ei),ni[2]),'float')*np.nan
...
for j in range(0,ni[2]):
    ...
    temp_interp_pres=si.UnivariateSpline(xp[::-1],yt[::-1],k=3)
    mixr_interp_pres=si.UnivariateSpline(xp[::-1],ym[::-1],k=3)
    u_interp_pres=si.UnivariateSpline(xp[::-1],yw[::-1],k=3)
    v_interp_pres=si.UnivariateSpline(xp[::-1],yd[::-1],k=3)
    relh_interp_pres=si.UnivariateSpline(xp[::-1],yr[::-1],k=3)
    ...
    # #print j, np.nanmax(yr), np.nanmax(relh_pres[:,j]), np.nanmax(yt),np.nanmax(temp_pres[:,j])

    # #print j, np.nanmin(yr), np.nanmin(relh_pres[:,j]), np.nanmin(yt),np.nanmin(temp_pres[:,j])


    # print j, np.nanmax(yr), np.nanmax(relh_pres), np.nanmax(yt),np.nanmax(temp_pres)
    # print j, np.nanmin(yr), np.nanmin(relh_pres), np.nanmin(yt),np.nanmin(temp_pres)

    # # print j, np.nanmax(relh_pres), np.nanmin(relh_pres),np.nanmax(temp_pres),np.nanmin(temp_pres)


    del xp, yr, yt, ym, yd, yw
...
plot(temp_pres[:,282],pres_ei,'-o')
plot(temp[:,282],pres[:,282],'-o')
#show()
...
hght_ei=hght_ei[4:21]
#*****************************************************************************\
for j in range(0,ni[2]):
#Calculate new variables
    #for i in range(0,len(hght_ei)):
    for i in range(0,len(pres_ei[4:21])):
...
#*****************************************************************************\
    ...
    for j in range(-10,10):
        for i in range(0, len(df)):
            if df['catdist_fron'][i]==j:
                RHmac[k1,:,j]=np.array(df[name_var_mac[m]][i])
...
#*****************************************************************************\
    ...
    n_rdyi=2 #Number of cases to take Yi 70
    ...
    for i in range(0,n_mean):
        ...
        RHD_rdmean[i,:,:]=np.nanmean(RH_rd,axis=0) #Random means dif
...
# Differences general means
    ...
    #RH_meandif_lev=RH_meandif[0:max_lev,:]

#*****************************************************************************\
#Means from MonteCarlo

    MM_rdmeanmac=RHD_rdmeanmac_yi
    MM_rdmeanera=RHD_rdmeanera_yi
    RHD_rdmean=RHD_rdmean
...
#*****************************************************************************\
    ...
    tmask=np.empty([max_lev,len(ncount)])*np.nan

    for lev in range(0,max_lev):
        for dis in range(0,len(ncount)):
            Xo=MG_meandif[lev,dis] #Media del total
            x=MM_rdmeanmac[:,lev,dis]-MM_rdmeanera[:,lev,dis]

            x_mean=np.nanmean(MM_rdmeanmac[:,lev,dis]-MM_rdmeanera[:,lev,dis])
            x_std=np.nanstd(MM_rdmeanmac[:,lev,dis]-MM_rdmeanera[:,lev,dis])
...
#*****************************************************************************\
    ...
    print np.nanmin(MG_mac), np.nanmax(MG_mac)
#*****************************************************************************\
...
#*****************************************************************************\
    ...
    img = ax.pcolor(tmask2[0:17,:],cmap=cmap)
...
#*****************************************************************************\
    ...
    RH_ssd=ma.masked_array((MG_mac-MG_era),mask=tmask)
    ...
    img = ax.pcolor(RH_ssd[0:17,:],cmap=cmap, vmin=z_mini, vmax=z_maxi)
    div = make_axes_locatable(ax)
...
#*****************************************************************************\
    ...
    fig, ax = plt.subplots(facecolor='w', figsize=(12,6))
    img = ax.pcolor(MG_mac[0:17,:],cmap='jet', vmin=z_min[m], vmax=z_max[m])
...
#*****************************************************************************\
    ...
    fig, ax = plt.subplots(facecolor='w', figsize=(12,6))
    img = ax.pcolor(MG_era[0:17,:],cmap='jet', vmin=z_min[m], vmax=z_max[m])
