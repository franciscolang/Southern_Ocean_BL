...
#Interpolation ERAI levels
    ...
    if max(x)<10000:
        ...
        rest=interp1d(x,yt,kind='cubic')(new_xt)
...
umac_ylev=pruu.reshape(-1,len(new_xt)).transpose()
vmac_ylev=pruv.reshape(-1,len(new_xt)).transpose()
mixrmac_ylev=prumixr.reshape(-1,len(new_xt)).transpose()
relhmac_ylev=prurelh.reshape(-1,len(new_xt)).transpose()
...
plt.grid(True)
ax0.semilogy(Tmacni, Pmacni, '-or',label='MAC NI')
ax0.semilogy(Tmac, Pmac, '-og',label='MAC')
