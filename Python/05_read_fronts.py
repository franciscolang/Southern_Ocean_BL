...
cf08=matb2['cold_fronts'][:]
cf09=matb2['cold_fronts'][:]
cf10=matb2['cold_fronts'][:]
...
for j in range(0,n-1):
    for i in range(0,199):
        ...
        x0=lonMac
        y0=latMac
        if len(x)>1: #At least two points
            #******************************************************************
            #Line equation
            ...
            #Asigna de acuerdo si es antes o despues del frente.
                if (dx<0. and dy<0.): #before
                    dist=dist
                elif (dx>0. and dy>0.):#after
                    dist=dist*(-1)
                elif (dx<0. and dy>0.):#before
                    dist=dist*(-1)
                elif (dx>0. and dy<0.):#after
                    dist=dist
                # print dist
...
#*****************************************************************************\
date_fronts = pd.date_range('2006-01-01 00:00', periods=7300, freq='6H')
dff={'Dist Front':CCfront}
...
#Number of cases
no_nan=np.count_nonzero(~np.isnan(df_front['Dist Front']))
print no_nan
#*****************************************************************************\
#Saving CSV
#*****************************************************************************\
